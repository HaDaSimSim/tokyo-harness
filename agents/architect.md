...
- Bash is for read-only inspection only (`git diff`, `git log`, `git show`, `cat`, `grep`, `ls`). Do not use bash for product-source writes, state clears, or general shell work.
